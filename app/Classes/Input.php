...
//        $item = urlencode($item);
    ...
    private static function get_param($methode = 'GET', $item, $type = null, $default = null, $arraytype = null) {
        ...
        if ($methode == 'post' && isset( $_POST[ $item ] )) $output = $_POST[ $item ];
        else if ($methode == 'get' && isset( $_GET[ $item ] )) $output = $_GET[ $item ];
        else if ($methode == 'session' && isset( $_SESSION[ $item ] )) $output = $_SESSION[ $item ];
        else if ($methode == 'cookie' && isset( $_COOKIE[ $item ] )) $output = $_COOKIE[ $item ];
        else if ($methode == 'files' && isset( $_FILES[ $item ] )) return $_FILES[ $item ];
        else $output = $default;
        ...
        switch (strtolower(strval($type))) {
            ...
            case 'string' :
                ...
                break;
            case 'array'  :
                if ($output && is_array($output)) foreach ($output as $key => $value) {
    ...
    public static function ip() {
        if (isset( $_SERVER[ 'HTTP_CLIENT_IP' ] ) && !empty( $_SERVER[ 'HTTP_CLIENT_IP' ] )) $ipaddress = $_SERVER[ 'HTTP_CLIENT_IP' ];
        else if (isset( $_SERVER[ 'HTTP_X_FORWARDED_FOR' ] ) && !empty( $_SERVER[ 'HTTP_X_FORWARDED_FOR' ] )) $ipaddress = $_SERVER[ 'HTTP_X_FORWARDED_FOR' ];
        else if (isset( $_SERVER[ 'HTTP_X_FORWARDED' ] ) && !empty( $_SERVER[ 'HTTP_X_FORWARDED' ] )) $ipaddress = $_SERVER[ 'HTTP_X_FORWARDED' ];
        else if (isset( $_SERVER[ 'HTTP_FORWARDED_FOR' ] ) && !empty( $_SERVER[ 'HTTP_FORWARDED_FOR' ] )) $ipaddress = $_SERVER[ 'HTTP_FORWARDED_FOR' ];
        else if (isset( $_SERVER[ 'HTTP_FORWARDED' ] ) && !empty( $_SERVER[ 'HTTP_FORWARDED' ] )) $ipaddress = $_SERVER[ 'HTTP_FORWARDED' ];
        else if (isset( $_SERVER[ 'REMOTE_ADDR' ] ) && !empty( $_SERVER[ 'REMOTE_ADDR' ] )) $ipaddress = $_SERVER[ 'REMOTE_ADDR' ];
    ...
    public static function protocol() {
        ...
        else if (isset( $_SERVER[ 'SERVER_PORT' ] ) && $_SERVER[ 'SERVER_PORT' ] == 443) return 'https';
        else if (isset( $_SERVER[ 'REQUEST_SCHEME' ] ) && $_SERVER[ 'REQUEST_SCHEME' ] == "https") return 'https';
        else if (isset( $_SERVER[ 'HTTP_X_FORWARDED_PROTO' ] ) && $_SERVER[ 'HTTP_X_FORWARDED_PROTO' ] == 'https') return 'https';
        else return 'http';
    }
    ...
    public static function file_name() {
        $filename = '';
        if(isset($_SERVER['SCRIPT_FILENAME'])) $filename = $_SERVER['SCRIPT_FILENAME'];
        else if(isset($_SERVER['SCRIPT_NAME'])) $filename = $_SERVER['SCRIPT_NAME'];
        else if(isset($_SERVER['PHP_SELF'])) $filename = $_SERVER['PHP_SELF'];
